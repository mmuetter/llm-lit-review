"""Classify the synerg* sample with one model, domain and outcome stages.

Usage: python run_sample.py mistral|sonnet
Mistral is dispatched in fast-to-slow waves, Sonnet through the standard
stage runner. Cells append to screening_v5/, so any restart resumes.
"""
...
run_screening.RESULTS_DIR = DATA_DIR / "screening_v5"
STAGE_TASKS = {"domain": [DOMAIN_TASK], "outcome": list(OUTCOME_PROMPTS)}
USAGE = "usage: python run_sample.py mistral|sonnet"
...
def run_mistral(papers):
    ...
    for stage, tasks in STAGE_TASKS.items():
        ...
        run_screening.banner(f"SAMPLE - Mistral {stage}", f"{len(units)} calls pending")
...
def main():
    """Run the requested model over the sample."""
    model = sys.argv[1] if len(sys.argv) > 1 else ""
    if model not in RUNNERS:
        sys.exit(USAGE)
    papers, payload = load_sample()
    run_screening.banner(f"SAMPLE - {model}",
                         f"{len(papers)} papers from a pool of {payload['population']}")
